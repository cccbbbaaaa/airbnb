"""
XGBoost Rating Prediction Model (Merged Data)
XGBoost 评分预测模型（整合数据版本）

整合 listings_detailed 和 listings_detailed_2 两个数据文件，然后进行评分预测
Merges listings_detailed and listings_detailed_2, then performs rating prediction
"""
...
def train_merged_rating_model():
    """训练整合数据版本的评分预测模型 / Train merged data rating prediction model."""
    print("=" * 80)
    print("XGBoost Rating Prediction Model (Merged Data)")
    print("XGBoost 评分预测模型（整合数据版本）")
    print("=" * 80)
    ...
    # 排除目标列和数据源列（数据源列可以保留作为分析用，但不作为特征）
    ...
    # 只保留有评分的记录
    mask = y.notna() & (y >= 0) & (y <= 100)
    X = X[mask].copy()
    y = y[mask].copy()
    
    print(f"  有效数据: {len(X)} 条记录")
    print(f"  评分统计: 均值={y.mean():.2f}, 中位数={y.median():.2f}, "
    ...
    output_importance = charts_dir / "xgboost_rating_feature_importance_merged.csv"
    ...
    axes[0, 0].set_title('Top 20 Feature Importance (Merged Data)')
    ...
    plt.tight_layout()
    output_chart = charts_dir / 'xgboost_rating_model_results_merged.png'
    ...
    # =========================================================================
    print("\n7. 保存模型 / Saving Model...")
    model_path = charts_dir / "xgboost_rating_model_merged.pkl"
    # 注意：XGBoost < 2.0.0 使用.pkl, >= 2.0.0 推荐 .json
    try:
        json_path = str(model_path).replace('.pkl', '.json')
        model.save_model(json_path)
        print(f"  [OK] 模型已保存到: {json_path}")
    except Exception:
        import pickle
...
def main():
    metrics = train_merged_rating_model()
    print("\n" + "=" * 80)
    print("最终指标 / Final Metrics:")
    for key, value in metrics.items():
        print(f"  {key}: {value:.4f}" if isinstance(value, float) else f"  {key}: {value}")
    print("=" * 80)
    print("整合数据评分预测模型完成 / Merged data rating model training complete")
    print("=" * 80)

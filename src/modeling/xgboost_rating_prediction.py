"""
XGBoost Review Rating Prediction Model
XGBoost 评分预测模型

本脚本使用XGBoost预测房源评分（review_scores_rating）
This script uses XGBoost to predict listing review scores (review_scores_rating)
"""
...
print("=" * 80)
print("XGBoost Review Rating Prediction Model")
print("XGBoost 评分预测模型")
print("=" * 80)
...
# 保存特征重要性
feature_importance.to_csv(charts_dir / 'xgboost_rating_feature_importance.csv', index=False)
print(f"\n  [OK] 特征重要性已保存到: {charts_dir / 'xgboost_rating_feature_importance.csv'}")
...
plt.tight_layout()
plt.savefig(charts_dir / 'xgboost_rating_model_results.png', dpi=300, bbox_inches='tight')
plt.close()

print(f"  [OK] 可视化图表已保存到: {charts_dir / 'xgboost_rating_model_results.png'}")
...
import pickle
model_path = charts_dir / 'xgboost_rating_model.pkl'
...
print("\n" + "=" * 80)
print("XGBoost评分预测模型训练完成！/ XGBoost Rating Prediction Model Training Complete!")
print("=" * 80)

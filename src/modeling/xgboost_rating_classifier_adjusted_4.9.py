"""
XGBoost Rating Classification Model (Adjusted 2021 Data) - Threshold 4.9
XGBoost 评分分类模型（使用调整后的2021年数据 - 阈值4.9）

参考 Rating_distribution.py 中的评分分布调整方法，调整2021年数据使其分布与2025年相似
然后整合调整后的2021年数据和2025年数据，使用XGBoost构建分类模型预测评分>4.9
...
classification model predicting rating > 4.9
"""
...
def train_adjusted_rating_classifier_4_9():
    """训练使用调整后数据的评分分类模型（阈值4.9）/ Train rating classification model with adjusted data (threshold 4.9)."""
    print("=" * 80)
    print("XGBoost Rating Classification Model (Adjusted 2021 Data - Threshold 4.9)")
    print("XGBoost 评分分类模型（使用调整后的2021年数据 - 预测高分评分>4.9）")
    print("=" * 80)
    ...
    # 创建二分类标签（评分>4.9为高分）
    processed_df['is_high_rating'] = (processed_df[target_col] > 4.9).astype(int)
    ...
    print(f"  高分样本数（评分>4.9）: {processed_df['is_high_rating'].sum():,} ({processed_df['is_high_rating'].mean():.2%})")
    print(f"  低分样本数（评分≤4.9）: {(~processed_df['is_high_rating'].astype(bool)).sum():,} ({(1-processed_df['is_high_rating'].mean()):.2%})")
    ...
    print(classification_report(y_test, y_test_pred, target_names=['Low Rating (≤4.9)', 'High Rating (>4.9)']))
    ...
    output_importance = charts_dir / "xgboost_rating_classifier_adjusted_4.9_importance.csv"
    ...
    axes[0, 0].set_title('Top 20 Feature Importance (Adjusted Data - Classification >4.9)')
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', ax=axes[1, 0],
                xticklabels=['Low Rating (≤4.9)', 'High Rating (>4.9)'],
                yticklabels=['Low Rating (≤4.9)', 'High Rating (>4.9)'])
    axes[1, 0].set_xlabel('Predicted')
    axes[1, 0].set_ylabel('Actual')
    axes[1, 0].set_title('Confusion Matrix')

    # 7.4 预测概率分布
    axes[1, 1].hist(y_test_proba[y_test == 0], bins=50, alpha=0.5, label='Low Rating (≤4.9)', color='red')
    axes[1, 1].hist(y_test_proba[y_test == 1], bins=50, alpha=0.5, label='High Rating (>4.9)', color='green')
    ...
    plt.tight_layout()
    output_chart = charts_dir / 'xgboost_rating_classifier_adjusted_4.9_results.png'
    ...
    model_path = charts_dir / "xgboost_rating_classifier_adjusted_4.9.json"
    try:
        model.save_model(str(model_path))
        print(f"  [OK] 模型已保存到: {model_path}")
    except Exception:
        import pickle
        pkl_path = charts_dir / "xgboost_rating_classifier_adjusted_4.9.pkl"
...
def main():
    try:
        metrics = train_adjusted_rating_classifier_4_9()
        if metrics is None:
            print("\n[ERROR] 模型训练失败，未返回指标")
            return
        ...
        print("=" * 80)
        print("使用调整后数据的评分分类模型完成（阈值4.9）/ Adjusted data rating classification model training complete (threshold 4.9)")
        print("=" * 80)
    except Exception as e:
        print(f"\n[ERROR] 程序执行失败: {e}")
        import traceback

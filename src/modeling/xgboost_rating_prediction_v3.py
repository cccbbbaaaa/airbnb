"""
XGBoost Rating Prediction Model (Enhanced Features v3)
XGBoost 评分预测模型（v3 - 包含情感分析和主机验证特征）

在 v2 版本基础上，使用 v2 特征工程脚本，增加了情感分析和主机验证等特征
Utilizes v2 feature engineering script, adding sentiment analysis, host verification, and other features.
"""
...
from rating_feature_engineering_v2 import engineer_rating_features_v2

setup_plotting()


def train_enhanced_rating_model_v3():
    """训练 v3 强化版评分预测模型 / Train v3 enhanced rating prediction model."""
    print("=" * 80)
    print("XGBoost Rating Prediction Model (v3 - Sentiment & Verification Features)")
    print("XGBoost 评分预测模型（v3 - 包含情感分析和主机验证特征）")
    print("=" * 80)

    # =========================================================================
    # 1. 构建特征 / Build Features
    # =========================================================================
    print("\n1. 构建 v2 特征 / Building v2 Features...")
    try:
    ...
    # 使用与 v2 相同的参数以便比较 / Use same params as v2 for comparison
    params = {
    ...
    output_importance = charts_dir / "xgboost_rating_feature_importance_v3.csv"
    ...
    # 6. 保存模型 / Save Model
    # =========================================================================
    model_path = charts_dir / "xgboost_rating_model_v3.pkl"
    # 注意：XGBoost < 2.0.0 使用.pkl, >= 2.0.0 推荐 .json
    # Note: XGBoost < 2.0.0 uses .pkl, >= 2.0.0 recommends .json
    try:
        model.save_model(str(model_path).replace('.pkl', '.json'))
        print(f"  [OK] 模型已保存到: {str(model_path).replace('.pkl', '.json')}")
    except Exception:
        model.save_model(model_path) # Fallback for older versions
        print(f"  [OK] 模型已保存到 (fallback): {model_path}")
...
def main():
    metrics = train_enhanced_rating_model_v3()
    print("\n最终指标 / Final Metrics:")
    for key, value in metrics.items():
        print(f"  {key}: {value:.4f}" if isinstance(value, float) else f"  {key}: {value}")
    print("=" * 80)
    print("v3 强化版评分预测模型完成 / v3 enhanced rating model training complete")
    print("=" * 80)

"""
XGBoost Rating Classification Model (Merged Data) - Threshold 4.9
XGBoost 评分分类模型（整合数据版本 - 阈值4.9）

整合 listings_detailed 和 listings_detailed_2，预测高分（评分>4.9）
Merges listings_detailed and listings_detailed_2, predicts high rating (rating > 4.9)
"""
...
def train_merged_rating_classifier_4_9():
    """训练整合数据版本的评分分类模型（阈值4.9）/ Train merged data rating classification model (threshold 4.9)."""
    print("=" * 80)
    print("XGBoost Rating Classification Model (Merged Data - Threshold 4.9)")
    print("XGBoost 评分分类模型（整合数据版本 - 预测高分评分>4.9）")
    print("=" * 80)
    ...
    output_importance = charts_dir / "xgboost_rating_classifier_merged_4.9_importance.csv"
    ...
    axes[0, 0].set_title('Top 20 Feature Importance (Merged Data - Classification >4.9)')
    ...
    plt.tight_layout()
    output_chart = charts_dir / 'xgboost_rating_classifier_merged_4.9_results.png'
    ...
    # =========================================================================
    print("\n7. 保存模型 / Saving Model...")
    model_path = charts_dir / "xgboost_rating_classifier_merged_4.9.json"
    try:
        model.save_model(str(model_path))
        print(f"  [OK] 模型已保存到: {model_path}")
    except Exception:
        import pickle
        pkl_path = charts_dir / "xgboost_rating_classifier_merged_4.9.pkl"
...
def main():
    metrics = train_merged_rating_classifier_4_9()
    print("\n" + "=" * 80)
    print("最终指标 / Final Metrics:")
    for key, value in metrics.items():
        print(f"  {key}: {value:.4f}" if isinstance(value, float) else f"  {key}: {value}")
    print("=" * 80)
    print("整合数据评分分类模型完成（阈值4.9）/ Merged data rating classification model training complete (threshold 4.9)")
    print("=" * 80)

...
# 注意：SVM在大数据集上训练较慢，这里使用较小的样本进行训练
# 如果数据量太大，可以考虑使用LinearSVR或减少训练样本
svm_model = SVR(
    kernel='rbf',
    C=1.0,  # 正则化参数
    epsilon=0.1,  # 不敏感损失函数的epsilon参数
    gamma='scale',  # RBF核的参数
    max_iter=5000,  # 增加最大迭代次数以避免收敛警告
    tol=1e-3  # 收敛容差，稍微放宽以加快收敛
)

print("  开始训练SVM模型（可能需要较长时间）...")
# 抑制收敛警告（如果达到最大迭代次数，模型仍然可以使用）
import warnings
